...
firepad.RichTextToolbar = (function(global) {
  ...
  function RichTextToolbar(imageInsertionUI, showEmojiPicker) {
  ...
  RichTextToolbar.prototype.makeButton_ = function(eventName, iconName) {
    ...
    utils.on(btn, 'mouseover', function() { mesh.showTooltip(`text-${eventName}`, btn); });
    utils.on(btn, 'mouseout', function() { mesh.hideTooltip(); });
    return btn;
  }
  ...
  RichTextToolbar.prototype.makeElement_ = function() {
    ...
    var toolbarOptions = [
      ...
      utils.elt('div', [self.makeButton_('ordered-list', 'numbered-list'), self.makeButton_('unordered-list', 'list-2'), self.makeButton_('todo-list', 'list')], { 'class': 'firepad-btn-group'}),
    ...
    if (self.showEmojiPicker) {
      var button = self.makeButton_('emoji');
      button.id = 'pick_emoji';
      toolbarOptions.push(utils.elt('div', [button], { 'class': 'firepad-btn-group' }));
    }
  ...
  RichTextToolbar.prototype.makeDropdown_ = function(title, eventName, items, value_suffix) {
    ...
    utils.on(button, 'mouseover', function() { mesh.showTooltip(`text-${eventName}`, button); });
    utils.on(button, 'mouseout', function() { mesh.hideTooltip(); });
